...
function createStream (opts) {
  ...
  for (let i = 0; i < addresses.length; ++i) {
    ...
    if (address.startsWith('ws://') || address.startsWith('wss://')) {
      ...
      return new WebSocketStream(new WebSocket(address));
    }
...
function createConnection (opts) {
  ...
  function onStreamConnect () {
    if (opts.noAuth) {
      afterHandshake(null, null);
    } else {
      clientHandshake(stream, opts, afterHandshake);
    }
  }
  stream.once('connect', onStreamConnect);
  stream.once('connected', onStreamConnect);
